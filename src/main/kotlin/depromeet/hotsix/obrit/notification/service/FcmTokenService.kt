...
class FcmTokenService(private val fcmTokenRepository: FcmTokenRepository) {
    ...
    private fun findOrCreateToken(userId: Long, token: String): FcmToken = fcmTokenRepository.findByToken(token)
        ?.also { it.reassignOwner(userId) }
        ?: createToken(userId, token)

    private fun createToken(userId: Long, token: String): FcmToken = try {
        fcmTokenRepository.saveAndFlush(FcmToken(userId = userId, token = token))
    } catch (e: DataIntegrityViolationException) {
        // 동시 요청으로 unique 제약 충돌 시 기존 토큰 소유자 재지정
        fcmTokenRepository.findByToken(token)?.also { it.reassignOwner(userId) } ?: throw e
    }
}
