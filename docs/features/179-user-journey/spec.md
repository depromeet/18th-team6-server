...
#### `item_search_performed`

- **트리거**: 클라이언트가 검색을 수행했을 때
- **발행 위치**: 신규 수집 API (`POST /analytics/search`)
...
**분석 활용**
- 검색 기능 사용률 (검색하는 사용자 비율)
- 결과 0건 검색어 → 사용자가 찾는데 없는 소모품·카테고리 파악
- 빈출 검색어 → 카테고리 프리셋 개선 근거

**수집 시 지킬 것**
- 검색어는 트림 후 저장하고, 길이 상한(100자)을 넘으면 잘라낸다
- 빈 문자열 검색은 발행하지 않는다
...
| 검색 사용 현황 | 검색 사용자 비율, 결과 0건 검색어, 빈출 검색어 |
...
| 3 | 검색 이벤트 수집 API | 호출 시 이벤트 저장 확인 |
| 4 | user-journey 화면·집계 | 화면 렌더링, 집계 로직 테스트 |
...
- [ ] `ocr_ms` 측정 구간을 `OcrService` 호출 전후로 할지, `ReceiptOcrClient` 내부로 좁힐지
- [ ] 검색 이벤트 수집 API의 인증 방식 — 기존 API와 동일하게 `X-User-Id` 헤더를 쓸지
