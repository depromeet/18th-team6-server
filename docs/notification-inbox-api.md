...
묶음·공지·기존 알림의 `label`, `nextReplacementDate`, `itemId`는 null이며 홈으로 이동한다.
묶음 알림의 표시 정보는 이번 범위에 포함하지 않는다.
기존 알림은 대상 소모품 ID가 저장되어 있지 않아 역산하지 않는다.

## 앱 연동

딥링크 기본값은 **앱 측 확인 전 임시 계약**이다.

- 단건: `obrit://items/{itemId}`
- 묶음·공지·기존 알림: `obrit://home`
...
알림함 클릭 시 읽음 API를 호출하고 응답의 `deepLink`로 이동할 수 있다.
푸시 클릭 시 `deepLink`로 이동한다. 배치 알림은 전송 성공 후 이력을 생성하므로 푸시에 알림 ID는 포함하지 않는다.

## DB

`V11__add_notification_item_snapshot.sql`이 nullable 컬럼 세 개를 추가한다.
기존 행은 null을 유지한다. 배포 시 애플리케이션 기동 전에 Flyway 적용이 필요하다.
